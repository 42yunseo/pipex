...
/*   Updated: 2024/04/14 12:53:51 by seoyungyo        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*nextline;
	static char	*readline[1048576];

	if (fd < 0 || BUFFER_SIZE < 1)
		return (NULL);
	readline[fd] = ft_readline(readline[fd], fd);
	if (readline[fd] == NULL)
		return (NULL);
	nextline = ft_nextline(readline[fd]);
	readline[fd] = ft_newreadline(readline[fd]);
	return (nextline);
}
